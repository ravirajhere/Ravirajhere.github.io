// ============================================
// FRIENDS.JS — IMPROVED & FIXED
// (Download Fixed + Debug Logs + Force Load)
// ============================================
...
function generateFriendCardPDF(isDBFriend) {
    console.log('📄 generateFriendCardPDF called!');
    console.log('📸 capturedPhotoData:', capturedPhotoData ? 'Yes ✅' : 'No ❌');
    console.log('👤 currentFriend:', currentFriend ? currentFriend.firstName : 'No ❌');
    console.log('📚 isDBFriend:', isDBFriend);
    ...
    if (typeof window.jspdf === 'undefined' && typeof window.jsPDF === 'undefined') {
        ...
        alert('❌ PDF library not loaded. Please check internet connection and try again.');
        return;
    }
    ...
    friendImg.onerror = function() {
        console.warn('⚠️ Friend image failed, using fallback');
        const canvas = document.createElement('canvas');
        canvas.width = 200;
        canvas.height = 200;
        const ctx = canvas.getContext('2d');
    ...
    formalImg.onerror = function() {
        console.warn('⚠️ Formal image failed, using fallback');
        const canvas = document.createElement('canvas');
        canvas.width = 200;
        canvas.height = 200;
        const ctx = canvas.getContext('2d');
    ...
    signImg.onerror = function() {
        console.warn('⚠️ Signature image failed, using fallback');
        const canvas = document.createElement('canvas');
        canvas.width = 200;
        canvas.height = 80;
        const ctx = canvas.getContext('2d');
    ...
    function generatePDF() {
        try {
            ...
            try {
                ...
                console.warn('⚠️ Could not add formal image:', e);
            }
            ...
            try {
                ...
                console.warn('⚠️ Could not add friend image:', e);
            }
